...
function getAvatarPath($userid,$remote=false,$cunityId=0) {
	global $cunity;
        /*
	$url = ($remote) ? $cunity->getCunityUrl($cunityId) : $cunity->getSetting("url");
    $res = $cunity->getDb()->query("SELECT i.file FROM ".$cunity->getConfig("db_prefix")."galleries_imgs as i,".$cunity->getConfig("db_prefix")."users AS u WHERE i.id = u.profile_image AND u.userid='".$userid."'") or die(mysql_error());    
	...
	$fileCheck = $_SESSION['cunity_trunk_folder'].$file;		
    if(file_exists($fileCheck)||$remote)
        return $file;
    else
        return 'style/'.$_SESSION['style'].'/img/no_avatar.jpg';*/
    if(file_exists("./files/_avatars/".  getUserHash($userid).".jpg"))
